...
### Governance
- Source PRs are open but **CI-gated**: schema valid, naming/taxonomy valid, scale tag present, referenced textures exist, eventually an automated parity render. *(Drift, 2026-09-23: the gates exist as `tools/validators/run_all.py` but are not wired into CI; that belongs to the one-command-check phase.)*
