...
3. **LCD as interface inputs.** Creator-adjustable values (tint, UV transform, overlay intensity, roughness bias) are exposed as **`<input>` interface ports on the material/nodegraph**, so a Creator tweak is a plain USD `inputs:` opinion overriding a shader input. See [LCDSchema](LCDSchema.md). UV placement rides a **`place2d`** nodegraph, not custom prim attrs.
