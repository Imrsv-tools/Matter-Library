...
| E5 | **Resolved (lead, 2026-09-25): `Natural`.** `Identity.md`'s "use `Base` if none" is to be updated when the naming rules land there. **The default Variant word.** `Identity.md` says "use `Base` if none"; this list uses `Natural` (untinted). | ~70 | Adopt `Natural` in `Identity.md`, or revert to `Base` (`WhiteOak_Base_Clean_Base`). |
