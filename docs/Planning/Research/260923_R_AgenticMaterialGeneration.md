...
| D8 | **Generated articles are "smart, not lean"** (lead, 2026-09-25). Every article carries all of its relevant overlays and masks, with sliders defaulted to 0; names don't change. So the skill must author a layer assignment per article, and the shared layer library must grow (5 → 22 in the draft). The ruling and its two contract collisions (the 2-overlay cap; overlays can't colour or smooth) are recorded in `260925_R_LibraryCoverage_FirstRelease.md` (ruling C1, Pass 4, L6–L8). |
