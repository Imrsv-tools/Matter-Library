...
| D8 | **Generated articles are "smart, not lean"** (lead, 2026-09-25). Every article carries all of its relevant overlays and masks, with sliders defaulted to 0; names don't change. So the skill must author a layer assignment per article, and the shared layer library must grow (5 → 22 in the draft). The overlay cap is raised from 2 to 3 (lead, same day), and one matter may have several articles with different layer sets. Both are recorded in `260925_R_LibraryCoverage_FirstRelease.md` (rulings C1–C3, Passes 4–5). The cap change is a contract change still to land. |
