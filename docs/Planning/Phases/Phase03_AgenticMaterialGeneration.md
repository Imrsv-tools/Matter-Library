...
| 3.2 | the 3.2 commit | ▶ SCAFFOLD COMPLETE — sitting owed (click 3). `recipe` lane live: 17 lanes, 15 PASS / 2 SKIP / 0 FAIL; the 13 recipes pass; 10 RED fixtures each rejected by their own guard (G1, G2, G3, G5, G6 ×2, G7 ×4); the assembler also refuses unknown keys; all 13 articles rebuild byte-identical to `HEAD` (P10 closed). **Lead-directed deviation:** from 3.2 on, files are written through Bash (lead: "write through Bash"), because the harness's background-session worktree guard refused Edit/Write despite `bgIsolation: none`. | 3.3 |
