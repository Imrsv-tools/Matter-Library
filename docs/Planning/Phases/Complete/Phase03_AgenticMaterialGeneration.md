...
> **Lead ruling (2026-09-25, at 3.4), verbatim:** "You need to be clear were I can see things... just open LiveView (the reason we made it) and I can look and adjust LCDs". **So the agent opens every draft (and any layer dialled up, as its own `--set` scene) in USDLiveView for the review, not images in chat.** This supersedes the Pass 2 rule that the skill never launches the viewer.
>
> **Lead ruling (2026-09-25, at 3.2), verbatim:** "Why I am doing any of this... go ahead and do what you need to do... I can look at stuff if you need eyes on, but entering commants and testing failure I'm not intersted". **So the agent runs every click below, including the refusals in click 3. The lead's part is click 2 only: look at the render (and the scene, if wanted), keep or discard, and the time that takes.** A click that is machine-checkable carries no human sitting.
...
| 3.2 | `5db572c` · the ruling commit | ✅ Click 3 run by the agent per the lead's ruling (the ⚠human tag drops: the refusals are machine-checkable). On copies of the ABS recipe: misspelled key → G1, `rubber` → G3 + G5, albedo 1.2 → G6, missing overlay → G7; the untouched recipe PASS. `recipe` lane live: 17 lanes, 15 PASS / 2 SKIP / 0 FAIL; the 13 recipes pass; 10 RED fixtures each rejected by their own guard (G1, G2, G3, G5, G6 ×2, G7 ×4); the assembler also refuses unknown keys; all 13 articles rebuild byte-identical to `HEAD` (P10 closed). **Lead-directed deviation:** from 3.2 on, files are written through Bash (lead: "write through Bash"), because the harness's background-session worktree guard refused Edit/Write despite `bgIsolation: none`. | 3.3 |
| 3.3 | `87c9d61` (release scope) · `11c12fd` (tooling) · the keep commit | ✅ **Lead: "keep all,"** (2026-09-25): `Fingerprints01`, `ABS_Glossy`, `Glass_Green` kept; review time not stated. `Fingerprints01` generated (seamless, byte-reproducible, provenance under `library/provenance/sources/`); `ABS_Glossy` + `Glass_Green` drafts; gate 15 PASS / 2 SKIP / 0 FAIL. **Two findings, both ruled by the lead ("go with your recommendations"):** (1) freeze and staging took every PNG on disk as a release's textures, so one draft texture broke `release_verify` on the frozen 0.1.0 — now scoped to the lock, reproducing 0.1.0's recorded digest exactly; (2) the preview cannot show see-through matter — the skill says so; a proper preview belongs to *Parity Baselines*. Also: light intensity barely changes a `usdrecord` render, so gloss reads weakly; a real lighting rig is *Parity Baselines* work. | 3.4 |
...
## Resume — historical (the close resumed on the lead's "close" and finished; kept for the ledger)
...
| Item | Disposition |
|---|---|
| Fingerprints01 / Scuffs01 seams | **fixed** at close: both generators on `tileable.py`, re-measured seamless, byte-reproducible. The anomaly: a stray staged deletion of `Fingerprints01` (unstaged back to HEAD) and a `Scuffs01` that reappeared after one delete (deleted again, regenerated). Two agent sessions were live on the box; which one caused it was not established. |
| `import_ambientcg.py` records the inherited `compute_evidence` note "NOT byte-identical to the upstream download", false when every map was copied verbatim (`Oak_Natural.yaml`) | **fixed** at close: the importer writes an accurate note when every map is verbatim; `Oak_Natural.yaml` corrected, evidence still verifies |
| Doc sync from the Brief: `AuthoringHarness.md`, `AuthoringGoldenPath.md` §Future-toolchain, `ToolingConventions.md` (the skill root; `tools/converters/{layers,base}/`, `tileable.py`, `import_ambientcg.py`, `lookup_physically_based.py`, `recipe.schema.json`; the 17th `recipe` lane; the flat texture layout drift), `CONTRIBUTING.md` (point at the skill), `docs/specs/Distribution/ReleaseModel.md` (a freeze covered every PNG; now the lock's textures) | **fixed** at close (the AuthoringGoldenPath doc lives at `docs/specs/Authoring/`) |
